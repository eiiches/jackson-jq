...
public class PipedQueryTest {
	...
	void test1() throws Exception {
		...
		assertEquals(Collections.emptyList(), q.apply(DefaultRootScope.getInstance(Versions.JQ_1_5), MAPPER.readTree("[11,22,33,44,55,66,77,88,99]")));
	}

	@Test
	void test2() throws Exception {
		final JsonQuery q = JsonQuery.compile("[label $out | foreach .[] as $item ([3, null]; if .[0] < 1 then break $out else [.[0] -1, $item] end; .[1])]");
		assertEquals(Collections.singletonList(MAPPER.readTree("[11,22,33]")), q.apply(DefaultRootScope.getInstance(Versions.JQ_1_5), MAPPER.readTree("[11,22,33,44,55,66,77,88,99]")));
	}
