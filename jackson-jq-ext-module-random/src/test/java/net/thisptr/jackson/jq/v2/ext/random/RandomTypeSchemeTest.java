...
/**
 * The signatures the {@code jackson-jq/random} functions publish, seen both directly and through the
 * types {@code TypeCheck} infers for calls to them.
 */
class RandomTypeSchemeTest {
	...
	@Test
	void anyInputAnswersAFloat() throws JsonQueryException {
		assertThat(outputOf("random::random", AnyType.getInstance()).toString()).isEqualTo("FLOAT");
		assertThat(outputOf("random::random", StringType.getInstance()).toString()).isEqualTo("FLOAT");
	}
}
