...
If you use Maven, add the following snippet to the `<dependencies>` section of your POM. For instructions for other build tools (Gradle, etc.), visit [jackson-jq](https://search.maven.org/artifact/net.thisptr/jackson-jq/1.6.3/jar) on search.maven.org.
...
```sh
$ curl -LO https://repo1.maven.org/maven2/net/thisptr/jackson-jq-cli/1.6.3/jackson-jq-cli-1.6.3.jar

$ java -jar jackson-jq-cli-1.6.3.jar --help
...
$ java -jar jackson-jq-cli-1.6.3.jar '.foo'
{"foo": 42}
42
```
...
```sh
$ java -jar jackson-jq-cli-1.6.3.jar --jq 1.5 'join("-")'
["1", 2]
jq: error: string ("-") and number (2) cannot be added

$ java -jar jackson-jq-cli-1.6.3.jar --jq 1.6 'join("-")' # jq-1.6 can join any values, not only strings
["1", 2]
"1-2"
```
...
```console
$ java -jar jackson-jq-cli-1.6.3.jar -n '1 + 3 | (. * 2)' # interpreted as (1 + 3) | (. * 2)
8
$ java -jar jackson-jq-cli-1.6.3.jar -n '1 + 3 as $a | ($a * 2)' # interpreted as (1 + 3) as $a | ($a * 2)
8
```
...
1
$ java -jar jackson-jq-cli-1.6.3.jar -n 'def f: 1; def g: f; def f: 2; g'
2
```
...
```console
$ java -jar jackson-jq-cli-1.6.3.jar -n 'def f1: 1; def g: f1; def f2: 2; g'
1
```
...
$ java -jar jackson-jq-cli-1.6.3.jar -n '0 / 0'
...
$ java -jar jackson-jq-cli-1.6.3.jar --jq 1.6 -n '[1,2,3] | ((.[] | select(. > 1)) |= empty)'
jq: error: `|= empty` is undefined. See https://github.com/stedolan/jq/issues/897
$ java -jar jackson-jq-cli-1.6.3.jar --jq 1.5 -n '[1,2,3] | ((.[] | select(. > 1)) |= empty)'
jq: error: `|= empty` is undefined. See https://github.com/stedolan/jq/issues/897
```
...
```console
$ java -jar jackson-jq-cli-1.6.3.jar --jq 1.6 -n '[1,2,3] | _modify((.[] | select(. > 1)); empty)'
[ 1, 3 ]
$ java -jar jackson-jq-cli-1.6.3.jar --jq 1.5 -n '[1,2,3] | _modify((.[] | select(. > 1)); empty)'
null
```
...
["foo"]
$ java -jar jackson-jq-cli-1.6.3.jar --jq 1.5 -c 'path(.foo as $a | $a)' <<< '{"foo": 1}'
...
$ java -jar jackson-jq-cli-1.6.3.jar --jq 1.6 -c 'path(.foo as $a | $a)' <<< '{"foo": 1}'
jq: error: Invalid path expression with result 1
```
...
$ java -jar jackson-jq-cli-1.6.3.jar -n 'label $a | label $b | try (break $b) catch .'
{
  "__jq" : 0
}
```
...
[]
$ java -jar jackson-jq-cli-1.6.3.jar -n '"x" | indices("")'
[ ]
```
